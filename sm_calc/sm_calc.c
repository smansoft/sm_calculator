...
#define COPYRIGHT			"Copyright (c) 2020-2021 SManSoft"
#define PRODUCT_NAME		"SM Calculator, Re-Entrant Parser Core"
...
FILE* gsm_yyout = NULL;					//	stream file, that is used for temprorary saving the output stream (is used by unit tests)
FILE* gsm_yyin = NULL;					//	stream file, that is used for temprorary saving the input stream (is used by unit tests)
...
const char* const	gcsm_s_format = "%s;";									//	print format: string and ';'; is used for printing the string result 
const char* const	gcsm_nl = "\n";											//	print format: new line

const char gcsm_help_fpath[] = { SM_BIN_PATH_MARKER ".." SM_FILES_DIR_SLASH "share" SM_FILES_DIR_SLASH "smansoft"
										SM_FILES_DIR_SLASH "doc" SM_FILES_DIR_SLASH "HELP" };	// help path
...
int main(int argc, char* argv[])
{
	...
	sm_parser_ctx parser_ctx;

	sm_calc_show_version();

	sm_init_parser_ctx(&parser_ctx);
	...
	while (TRUE)
	{
		...
		switch (opt_val) {
		case 'l': {
			err_safe = safe_strcpy(parser_ctx.m_calc_params.m_log_file_path, SM_ARRAY_SIZE(parser_ctx.m_calc_params.m_log_file_path), optarg);
			if (err_safe) {
				sm_print_error(&parser_ctx, "wrong value of option '-l --log_file', default log path will be used");
				fprintf(stdout, "\n\n");
			}
			break;
		}
		case 'q': {
			parser_ctx.m_calc_params.m_no_log = TRUE;
			break;
		}
		}
	}
...
#ifndef UNIT_TESTING
	sm_init_log(&parser_ctx);

	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "%s %s", __FUNCTION__, "---------------------------------------------------------------------- >>");
	sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "sm_calc_re is started");
	...
	while (err == SM_RES_OK)
	{
		...
		case 'h': {
			...
			parser_ctx.m_calc_params.m_help = TRUE;
			break;
		}
		case 'i': {
			sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "opt_val == 'i' ('interactive')");
			parser_ctx.m_calc_params.m_interact_proc = TRUE;
			break;
		}
		case 'l': {
		...
		case 'f': {
			...
			err_safe = safe_strcpy(parser_ctx.m_calc_params.m_in_file_path, SM_ARRAY_SIZE(parser_ctx.m_calc_params.m_in_file_path), optarg);
			if (err_safe) {
				err = SM_RES_ERROR;
				sm_print_error(&parser_ctx, "wrong value of option '-f --in_file'");
			}
			break;
		}
		case 'o': {
			sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "opt_val == 'o' ('out_file')");
			err_safe = safe_strcpy(parser_ctx.m_calc_params.m_out_file_path, SM_ARRAY_SIZE(parser_ctx.m_calc_params.m_out_file_path), optarg);
			if (err_safe) {
				err = SM_RES_ERROR;
				sm_print_error(&parser_ctx, "wrong value of option '-o --out_file'");
			}
			break;
		}
		case 'x': {
			...
			if (!(len = safe_strnlen(optarg, SM_L_BUFF_SIZE - 1))) {
				sm_print_error(&parser_ctx, "wrong value of option '-x --expr'");
				err = SM_RES_ERROR;
				break;
			}
			err_safe = safe_strcpy(parser_ctx.m_calc_params.m_expression, SM_ARRAY_SIZE(parser_ctx.m_calc_params.m_expression) - 1, optarg);
			if (err_safe) {
				err = SM_RES_ERROR;
				sm_print_error(&parser_ctx, "wrong value of option '-x --expr'");
				break;
			}
			len = safe_strnlen(parser_ctx.m_calc_params.m_expression, SM_ARRAY_SIZE(parser_ctx.m_calc_params.m_expression) - 1);
			if (len) {
				if (parser_ctx.m_calc_params.m_expression[len - 1] != ';') {
					err_safe = safe_strcat(parser_ctx.m_calc_params.m_expression, SM_L_BUFF_SIZE, ";");
					if (err_safe) {
						err = SM_RES_ERROR;
						sm_print_error(&parser_ctx, "wrong value of option '-x --expr'");
					}
				...
				if (len > 0 && len <= 2) {
					...
					if (err == SM_RES_OK) {
						...
						if (SM_PREC_MIN <= precision && precision <= SM_PREC_MAX)
							parser_ctx.m_calc_params.m_f_precision = precision;
						else {
							err = SM_RES_ERROR;
							sm_print_error(&parser_ctx, "wrong value of option '-p --precision'");
						}
					}
					else
						sm_print_error(&parser_ctx, "wrong value of option '-p --precision'");
		...
		case 'n': {
			...
			for (int i = SM_I_FORMAT_MIN; i <= SM_I_FORMAT_MAX; i++) {
				...
				if (!err_safe && !cmp_res) {
					parser_ctx.m_calc_params.m_i_format = i;
					break;
				}
			}
			if (cmp_res)
				sm_print_error(&parser_ctx, "wrong value of option '-n --nformat'");
			break;
		}
		case 'a': {
			sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "opt_val == 'a' ('a_unit')");
			int cmp_res = -1;
			parser_ctx.m_calc_params.m_trig_unit = SM_TRIG_RADIANS;
			for (int i = SM_TRIG_UNIT_MIN; i <= SM_TRIG_UNIT_MAX; i++) {
				...
				if (len > 0) {
					...
					if (!err_safe && !cmp_res) {
						parser_ctx.m_calc_params.m_trig_unit = (sm_trig_unit)i;
						break;
					}
				}
			}
			if (cmp_res) {
				sm_print_error(&parser_ctx, "wrong value of option '-a --aunit'");
				err = SM_RES_ERROR;
			}
			break;
		}
	...
	sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "sm_calc_re is finished");
	sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "main ---------------------------------------------------------------------- <<");

	sm_close_log(&parser_ctx);
#endif /* UNIT_TESTING */
...
{
	errno_t err = SM_RES_OK;
	if (parser_ctx->m_calc_params.m_no_log) {		//	initializing the log will not provided, if user defined no_log (gsm_calc_params.m_no_log)
		gsm_log_config.m_start = 0;		//	via command line options
		return err;
	}
	size_t log_file_path_len = safe_strnlen(parser_ctx->m_calc_params.m_log_file_path, MAX_PATH);
	if (log_file_path_len > 0)	//	if user has defind gsm_calc_params.m_log_file_path (via command line) - init log, using this path (gsm_calc_params.m_log_file_path)
		err = sm_log_init_fpath(SM_LOG_CONFIG, parser_ctx->m_calc_params.m_log_file_path);
...
{
	if (parser_ctx->m_calc_params.m_no_log)		//	if user defined no_log (gsm_calc_params.m_no_log) via command line options, other activity
...
errno_t	sm_log_calc_params(sm_parser_ctx* const parser_ctx)
{
	...
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_interact_proc = %d", parser_ctx->m_calc_params.m_interact_proc);
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_cmd_line_proc = %d", parser_ctx->m_calc_params.m_cmd_line_proc);
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_file_proc     = %d", parser_ctx->m_calc_params.m_file_proc);
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_help          = %d", parser_ctx->m_calc_params.m_help);

	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_in_file_path = %s",
		parser_ctx->m_calc_params.m_in_file_path ? parser_ctx->m_calc_params.m_in_file_path : "NULL");
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_out_file_path = %s",
		parser_ctx->m_calc_params.m_out_file_path ? parser_ctx->m_calc_params.m_out_file_path : "NULL");
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_expression = %s",
		parser_ctx->m_calc_params.m_expression ? parser_ctx->m_calc_params.m_expression : "NULL");

	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_in_file  = 0x%x", parser_ctx->m_calc_params.m_in_file);
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_out_file = 0x%x", parser_ctx->m_calc_params.m_out_file);

	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_trig_unit   = %d", parser_ctx->m_calc_params.m_trig_unit);
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_f_precision = %d", parser_ctx->m_calc_params.m_f_precision);

	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_i_format = %d", parser_ctx->m_calc_params.m_i_format);
...
errno_t	sm_log_calc_res(sm_parser_ctx* const parser_ctx)
{
	...
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_res.m_sz_message = %s", parser_ctx->m_calc_res.m_sz_message);
...
{
	safe_memset(parser_ctx, sizeof(sm_parser_ctx), 0);

	sm_set_def_calc_params(parser_ctx);

	yyscan_t scanner = NULL;
	yylex_init_extra(parser_ctx, &scanner);
	parser_ctx->m_scanner = scanner;
	sm_set_yyin(scanner, stdin);
	sm_set_yyout(scanner, stdout);
...
errno_t sm_set_def_calc_params(sm_parser_ctx* const parser_ctx)
{
	...
	parser_ctx->m_calc_params.m_f_precision = SM_PREC_DEF;
	parser_ctx->m_calc_params.m_i_format = SM_I_FORMAT_DEF;
	parser_ctx->m_calc_params.m_trig_unit = SM_TRIG_UNIT_DEF;

	parser_ctx->m_calc_params.m_interact_proc = FALSE;
	parser_ctx->m_calc_params.m_cmd_line_proc = FALSE;
	parser_ctx->m_calc_params.m_file_proc = FALSE;
	parser_ctx->m_calc_params.m_help = FALSE;
	parser_ctx->m_calc_params.m_no_log = FALSE;

	while (TRUE) {
		err_safe = safe_strcpy(parser_ctx->m_calc_params.m_f_print_formats[0],
			SM_ARRAY_SIZE(parser_ctx->m_calc_params.m_f_print_formats[0]), "%Le");
		if (err_safe) {
			err = SM_RES_ERROR;
			break;
		}
		err_safe = safe_strcpy(parser_ctx->m_calc_params.m_f_print_formats[1],
			SM_ARRAY_SIZE(parser_ctx->m_calc_params.m_f_print_formats[1]), "");
		if (err_safe) {
			err = SM_RES_ERROR;
			break;
		}
...
errno_t	sm_proc_params(sm_parser_ctx* const parser_ctx)
{
	...
	while (1) {
		len = safe_strnlen(parser_ctx->m_calc_params.m_in_file_path, MAX_PATH);
		if (!len) {
			sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "length of parser_ctx->m_calc_params.m_in_file_path == 0");
			break;
		}
		sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "length of parser_ctx->m_calc_params.m_in_file_path > 0");
		err_safe = safe_fopen(&parser_ctx->m_calc_params.m_in_file, parser_ctx->m_calc_params.m_in_file_path, "r", SM_SFOPEN_SHARED_TYPE);
		if (err_safe == SAFE_RES_ERROR) {
			err = SM_RES_ERROR;
			char err_buf[SM_L_BUFF_SIZE];
			snprintf(err_buf, SM_ARRAY_SIZE(err_buf), "input file: %s hasn't been opened for reading", parser_ctx->m_calc_params.m_in_file_path);
			sm_print_error(parser_ctx, err_buf);
			break;
		}
		else
			sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "input file: %s has been opened for reading", parser_ctx->m_calc_params.m_in_file_path);
		len = safe_strnlen(parser_ctx->m_calc_params.m_out_file_path, MAX_PATH);
		if (!len) {
			sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "length of parser_ctx->m_calc_params.m_out_file_path == 0");
			break;
		}
		sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "length of gsm_calc_params.m_out_file_path > 0");
		err_safe = safe_fopen(&parser_ctx->m_calc_params.m_out_file, parser_ctx->m_calc_params.m_out_file_path, "w", SM_SFOPEN_SHARED_TYPE);
		if (err_safe == SAFE_RES_ERROR) {
			err = SM_RES_ERROR;
			char err_buf[SM_L_BUFF_SIZE];
			snprintf(err_buf, SM_ARRAY_SIZE(err_buf), "output file: %s hasn't been opened for writing", parser_ctx->m_calc_params.m_out_file_path);
			sm_print_error(parser_ctx, err_buf);
			break;
		}
		else
			sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "input file: %s has been opened for writing", parser_ctx->m_calc_params.m_in_file_path);
		break;
	}

	if (err == SM_RES_OK && parser_ctx->m_calc_params.m_in_file)
		parser_ctx->m_calc_params.m_file_proc = (parser_ctx->m_calc_params.m_in_file != NULL);
	else
		parser_ctx->m_calc_params.m_file_proc = FALSE;

	if (err == SM_RES_OK) {
		len = safe_strnlen(parser_ctx->m_calc_params.m_expression, SM_L_BUFF_SIZE);
		parser_ctx->m_calc_params.m_cmd_line_proc = len > 0;
		if (!parser_ctx->m_calc_params.m_interact_proc)
			parser_ctx->m_calc_params.m_interact_proc = !parser_ctx->m_calc_params.m_file_proc && !parser_ctx->m_calc_params.m_cmd_line_proc;
	}

	if (err == SM_RES_OK)
		err = proc_help_fpath(parser_ctx);
...
errno_t sm_init_f_prec_buf(sm_parser_ctx* const parser_ctx)
{
	...
	if (parser_ctx->m_calc_params.m_f_precision == SM_PREC_EXP) {
		sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_f_precision == SM_PREC_EXP");
		parser_ctx->m_calc_params.m_f_prec_buf = parser_ctx->m_calc_params.m_f_print_formats[0];
		sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_f_prec_buf = %s", parser_ctx->m_calc_params.m_f_prec_buf);
	}
	else {
		int prn_res = snprintf(parser_ctx->m_calc_params.m_f_print_formats[1], 
			SM_ARRAY_SIZE(parser_ctx->m_calc_params.m_f_print_formats[1]),
			"%c.%dLf", '%', parser_ctx->m_calc_params.m_f_precision);
		if (prn_res > 0) {
			parser_ctx->m_calc_params.m_f_prec_buf = parser_ctx->m_calc_params.m_f_print_formats[1];
			sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "parser_ctx->m_calc_params.m_f_prec_buf = %s", parser_ctx->m_calc_params.m_f_prec_buf);
			err = SM_RES_OK;
		}
		else
...
{
	errno_t err = SM_RES_OK;
	if (parser_ctx->m_calc_params.m_help) {
		sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "Help (sm_calc_usage):");
		sm_calc_usage();
		return err;
	}
	if (parser_ctx->m_calc_params.m_file_proc) {
		sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "Files processing is started");
		sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "in file  = %s", parser_ctx->m_calc_params.m_in_file_path);
		sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "out file = %s", parser_ctx->m_calc_params.m_out_file_path);
		printf("\tFiles processing:\n");
		printf("in file = %s\n", parser_ctx->m_calc_params.m_in_file_path);
		printf("out file = %s\n", parser_ctx->m_calc_params.m_out_file_path);
		err = sm_do_calc_files(parser_ctx, parser_ctx->m_calc_params.m_in_file, parser_ctx->m_calc_params.m_out_file);
		printf("\n");
		if (parser_ctx->m_calc_params.m_in_file)
			fclose(parser_ctx->m_calc_params.m_in_file);
		if (parser_ctx->m_calc_params.m_out_file)
			fclose(parser_ctx->m_calc_params.m_out_file);
		parser_ctx->m_calc_params.m_in_file = NULL;
		parser_ctx->m_calc_params.m_out_file = NULL;
		sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "Files processing: Result = %s", (err == SM_RES_OK) ? "Ok" : "Error");
	}
	if (parser_ctx->m_calc_params.m_cmd_line_proc) {
		sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "Command line processing is started");
		sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "expression  = %s", parser_ctx->m_calc_params.m_expression);
		printf("\tCommand line processing:\n");
		err = sm_do_calc_expression(parser_ctx);
		printf("\n");
	...
	if (parser_ctx->m_calc_params.m_interact_proc) {
		...
		sm_do_calc_interactive(parser_ctx);
		printf("\n");
	}
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "Result = %d", err);
	return err;
...
errno_t sm_do_calc_interactive(sm_parser_ctx* const parser_ctx)
{
	...
	int cycle = 0;
	BOOL out = FALSE;
	sm_set_yyin(parser_ctx->m_scanner, gsm_yyin);
	sm_set_yyout(parser_ctx->m_scanner, gsm_yyout);
	sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "Interactive processing");
	do {
		SM_SET_CALC_RES_OK;
		if (cycle > 0 && out)
			fprintf(sm_get_yyout(parser_ctx->m_scanner), "\n");
		out = FALSE;
		fprintf(sm_get_yyout(parser_ctx->m_scanner), "In  >> ");
		in_res = safe_gets(sm_get_yyin(parser_ctx->m_scanner), in_buf, SM_ARRAY_SIZE(in_buf) - 1);
		if (!in_res || !(in_len = safe_strnlen(in_buf, SM_L_BUFF_SIZE)))
			continue;
		if (in_buf[in_len - 1] != ';')
			safe_strcat(in_buf, SM_ARRAY_SIZE(in_buf), ";");
		fprintf(sm_get_yyout(parser_ctx->m_scanner), "Out >> ");
		...
		sm_scan_string(parser_ctx->m_scanner, in_buf);
		err = yyparse(parser_ctx->m_scanner, parser_callback);
		fprintf(sm_get_yyout(parser_ctx->m_scanner), "\n");
		sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "Interactive processing: yyparse res = %d", err);
		out = TRUE;
		cycle++;
	} while (!parser_ctx->m_calc_params.m_exit);
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "Interactive processing: parser_ctx->m_calc_params.m_exit = %d", parser_ctx->m_calc_params.m_exit);
...
errno_t sm_do_calc_expression(sm_parser_ctx* const parser_ctx)
{
	...
	sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "Expression processing: gsm_calc_params.m_expression = %s", parser_ctx->m_calc_params.m_expression);
	fprintf(sm_get_yyout(parser_ctx->m_scanner), "In  >> %s\nOut >> ", parser_ctx->m_calc_params.m_expression);
	sm_scan_string(parser_ctx->m_scanner, parser_ctx->m_calc_params.m_expression);
	err = yyparse(parser_ctx->m_scanner, parser_callback);	//	lexical analyzer
	fprintf(sm_get_yyout(parser_ctx->m_scanner), "\n");
...
{
	errno_t err = SM_RES_OK;
	sm_set_yyin(parser_ctx->m_scanner, in_file);
	sm_set_yyout(parser_ctx->m_scanner, out_file);
	sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "Files processing");
	sm_scan_file(parser_ctx->m_scanner, in_file);
	err = yyparse(parser_ctx->m_scanner, parser_callback);	//	lexical analyzer
	fprintf(out_file, "\n");
...
void sm_calc_usage()
{
	...
	printf("USAGE: sm_calc_re [OPTION]\n\n");
	...
	sm_log_print(SM_LOG_CONFIG, __FUNCTION__, "USAGE: sm_calc_re [OPTION]");
...
{
	errno_t err;
	err = sm_make_path_abs(parser_ctx->m_calc_params.m_help_fpath, SM_ARRAY_SIZE(parser_ctx->m_calc_params.m_help_fpath), gcsm_help_fpath, SM_BIN_PATH_MARKER);
	if (err == SM_RES_OK) {
		sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "gcsm_help_fpath = %s", gcsm_help_fpath);
		sm_log_printf(SM_LOG_CONFIG, __FUNCTION__, "gsm_help_fpath = %s", parser_ctx->m_calc_params.m_help_fpath);
	}
